...
public class AES256 extends BlockCipher {
    ...
    private final static String NAME = "AES256";
    ...
    public void initialize(Mode mode, byte[] key, int ofs) {
        super.initialize(mode, null, 0);
        this.workingKey = generateWorkingKey(key, ofs, KEY_SIZE);
    }

    @Override
    public void processBlock(byte[] in, int ofsi, byte[] out, int ofso) {
        if (Mode.ENCRYPT == this.mode) {
            unpackBlock(in, ofsi);
    ...
    private final static int shift(int r, int shift) {
    ...
    private final static int FFmulX(int x) {
        return (((x & m2) << 1) ^ (((x & m1) >>> 7) * m3));
    }

    private final static int inv_mcol(int x) {
        int f2 = FFmulX(x);
        int f4 = FFmulX(f2);
        int f8 = FFmulX(f4);
        int f9 = x ^ f8;
    ...
    private final static int subWord(int x) {
        return (S[ x         & 255] & 255         |
              ((S[(x >>>  8) & 255] & 255) <<  8) |
              ((S[(x >>> 16) & 255] & 255) << 16) |
                S[ x >>> 24       ]        << 24);  // !optimized
    ...
    @Override
    public Object clone() {
        AES256 result = new AES256();
        result.mode = this.mode;
        result.rounds = this.rounds;
        result.workingKey = BinUtils.copyArray(this.workingKey);
        result.C0 = this.C0;
        result.C1 = this.C1;
        result.C2 = this.C2;
        result.C3 = this.C3;
    ...
    private final int[][] generateWorkingKey(byte[] key, int ofs, int len) {
        int KC = len >> 2;
        int t;
    ...
    private final static int unpack(byte[] bytes, int index) {
    ...
    private final static void pack(int val, byte[] bytes, int index) {
    ...
    private final static long[] S_packed = {
    ...
    private final static long[] Si_packed = {
    ...
    private final static byte[] S  = BinUtils.longsToBytesLE(S_packed);
    private final static byte[] Si = BinUtils.longsToBytesLE(Si_packed);
    ...
    private final static int[] Tinv0 = {
    ...
    private final static int[] Tinv1 = {
    ...
    private final static int[] Tinv2 = {
    ...
    private final static int[] Tinv3 = {
    ...
    public void test() throws Throwable {
        ...
        final byte[] REF_CTXT = {
            (byte)0x8e, (byte)0xa2, (byte)0xb7, (byte)0xca, 0x51, 0x67, 0x45, (byte)0xbf, (byte)0xea, (byte)0xfc, 0x49, (byte)0x90, 0x4b, 0x49, 0x60, (byte)0x89
        };

        aes.initialize(Mode.ENCRYPT, TEST_KEY, 0);

        byte[] ctxt = new byte[REF_CTXT.length];

        aes.processBlock(REF_PTXT, 0, ctxt, 0);

        if (!BinUtils.arraysEquals(ctxt, REF_CTXT)) {
            throw new Exception();
        }

        byte[] btxt = new byte[REF_CTXT.length];
        aes.erase();
        aes.processBlock(REF_PTXT, 0, btxt, 0);

        if (BinUtils.arraysEquals(btxt, REF_CTXT)) {
            throw new Exception();
        }

        aes.initialize(Mode.DECRYPT, TEST_KEY, 0);

        byte[] ptxt = new byte[REF_PTXT.length];

        aes.processBlock(ctxt, 0, ptxt, 0);

        if (!BinUtils.arraysEquals(ptxt, REF_PTXT)) {
            throw new Exception();
        }
    }
}
